...
Права на `storage/` и `bootstrap/cache` управляются автоматически:

1. **Контейнер `backend_scheduler`** при старте выполняет:
   ```bash
   chown -R www-data:www-data /app/storage
   chmod -R g+rwX /app/storage
   find /app/storage -type d -exec chmod g+s {} +   # SGID для наследования группы
   ```

2. **Конфигурация `filesystems.php`** задаёт права для новых файлов/директорий:
   ```php
   'permissions' => [
...
3. **Контроллер** явно устанавливает `chmod(0775)` на temp-каталоги после загрузки файлов, чтобы Queue Worker (www-data) мог перемещать файлы из temp в постоянное хранилище.

**Если проблема всё равно возникает** — пересоздайте контейнеры:
