...
├── scripts/
│   ├── rebuild-frontend.sh       # Пересборка frontend без кеша
│   └── rebuild-telegram-bot.sh   # Пересборка Telegram-бота
...
### Rootless Podman: права доступа

При использовании rootless Podman контейнер может не иметь доступа к примонтированным директориям (CORS 403, "Permission denied"). Это происходит из-за user namespace mapping — `www-data` (uid 33) внутри контейнера маппится на subuid (524320) на хосте.

**Решение:** Скрипт настройки прав через ACL (без sudo):
...
Скрипт делает:
1. **Владелец всех файлов = хост-юзер** → git, редактор, всё работает
2. **ACL для www-data** → контейнер читает код, пишет в storage

| Путь | Владелец | ACL www-data | chmod |
|------|----------|--------------|-------|
| Исходный код | temur | `r-X` (чтение) | `755`/`644` |
| `storage/` | temur | `rwX` (запись) | `755`/`644` |
| `bootstrap/cache/` | temur | `rwX` (запись) | `755`/`644` |
| `.env` | temur | `r` (чтение) | `640` |
| `artisan` | temur | `r-X` (исполнение) | `755` |

Если контейнеры уже запущены:
```bash
./scripts/fix-permissions.sh
podman compose restart svc-api svc-scheduler svc-worker-cleanup svc-worker-proof svc-worker-shared svc-worker-generator
```

Проверка ACL: `getfacl TaskMateServer/storage`
...
| CORS 403 / Permission denied в rootless Podman | `./scripts/fix-permissions.sh` затем `podman compose restart svc-api` |
